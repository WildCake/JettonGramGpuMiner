...
echo "One GPU detected. Creating start file"
    ...
    while true; do
    node send_universal.js --api tonapi --bin ./pow-miner-cuda --givers 1000 --timeout 6
    sleep 0.6;
    done;
EOL
else
    ...
    while true; do
    node send_multigpu.js --api tonapi --bin ./pow-miner-cuda --givers 1000 --timeout 5 --gpu-count ${GPU_COUNT} 
    sleep 0.5;
    done;
EOL
fi
